...
# Start MCP server in stdio mode (for Claude Code etc)
dev:
...
release TYPE="patch":
    ...
    echo "Monitor at: https://github.com/hongkongkiwi/truenas-master-mcp/actions"
...
release-notes:
    ...
    echo "### Security"
    echo ""
    echo "**Full Changelog**: https://github.com/hongkongkiwi/truenas-master-mcp/compare/$(git describe --tags --abbrev=0 2>/dev/null || echo "previous")...v$(grep '^version = ' Cargo.toml | head -1 | sed 's/version = "\(.*\)"/\1/')
...
release-local:
    ...
    cargo build --release
    echo ""
    echo "Binary built at: target/release/truenas-master-mcp"
    echo ""
...
install-github VERSION="":
    ...
    echo "Installing truenas-master-mcp v$VERSION from source..."
    cargo install --git https://github.com/hongkongkiwi/truenas-master-mcp --tag "v$VERSION" --locked
...
help:
    @echo "TrueNAS Master MCP - Available Commands"
    @echo ""
    @echo "Build:"
